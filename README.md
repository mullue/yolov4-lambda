...
- Resource > HelloWorldFunction > Properties 섹션에서 `Layers`와 `MemorySize`, `Policies`를 추가합니다.
...
- Global > Function > 섹션의 `Timeout`을 60 이상으로 세팅합니다. 
...
- 추론용 코드는 원본의 `detect.py`를 바탕으로 Lambda환경에 맞게 재수정함
